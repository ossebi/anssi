...
export default function Index({ articleData }: SectionProps) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex justify-between rounded-xl p-4">
                ...
                <Link className='cursor-pointer px-3 py-2 text-xs font-medium text-white bg-blue-700 rounded hover:bg-blue-800'
                    href={sections.articles.create(articleData.slug)}>
                    Ajouter une section
                </Link>
            </div>
            <div className="overflow-hidden bg-white p-4">
                <table className="min-w-full border border-gray-200">
                    ...
                    <tbody>
                        {articleData.sectionsData?.map((item) => (
                            <tr key={item.id} className="border-t">
                                <td className="px-4 py-2 text-sm font-semibold">{item.title}</td>
                                <td className="px-4 py-2 text-sm font-semibold">{item.type}</td>
                                ...
                                <td className="px-4 py-2 text-sm font-semibold"> {item.status ? 'Actif' : 'Inactif'}</td>

                                <td className="px-4 py-2 text-sm">
                                    <div className='flex gap-x-1'>
                                        <Link href={sections.articles.show(item.id)} className="mr-2 text-blue-600 hover:underline" title='Voir'>
                                            <Eye className='w-5' />
                                        </Link>
                                        <Link href={sections.articles.edit([articleData.slug, item.id])} className="mr-2 text-green-600 hover:underline" title='Modifier'>
                                            <Edit className='w-5' />
                                        </Link>
                                        <Link
                                            as="button"
                                            method="delete"
                                            href={sections.articles.destroy.url(item.id)}
                        ...
                        {articleData.sectionsData?.length === 0 && (
                            <tr>
